...
abstract class Client(brokers: List[String], agentId: AgentId, bankAgentId: AgentId)( var moneyOnme: Double, accountId: String) extends AvroAgent[Transaction](defaultConfig(brokers = brokers, agentId = agentId))(null,"blabla") 
    with ClientLike {
    ...
    def  withdrawal(amount: Double) = {
        this.send(bankAgentId,
            Transaction(
                label = Some(s"$agentId-withdrawal"),
                from = Some(s"$agentId"),
                to = Some(s"$agentId"),
                amount = amount,
