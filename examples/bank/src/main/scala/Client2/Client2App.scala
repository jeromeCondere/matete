...
class Client2(brokers: List[String], bankAgentId: AgentId)
    extends Client(brokers, AgentId("Client2"), bankAgentId)(
      234,
      "Client2_account"
    )
    with Runnable {
  ...
  override def receive(
      agentMessages: List[AgentMessage[Transaction]],
      consumerName: String
  ) = {
    logger.info("AYO " + agentMessages)

    agentMessages
      .filter(_.message.typeTransaction == "deposit")
      .map(_.message)
      .foreach(
        transaction => {
          ...
          if (transaction.label.contains("deposit accepted")) {
            ...
            this.logger
              .info(s"Put deposit $amount on my account, I'm very seious")
    ...
    agentMessages
      .filter(_.message.typeTransaction == "withdrawal")
      .map(_.message)
      .foreach(
        transaction => {
          val amount = transaction.amount
          if (transaction.label.contains("withdrawal accepted")) {
            this.moneyOnme = this.moneyOnme + amount
            this.logger.info(
              s"The withdrawal of $amount has been accepted for my account, I'm serious"
            )
            logger.info("money " + moneyOnme)
          }
        }
