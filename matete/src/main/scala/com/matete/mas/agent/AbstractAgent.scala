...
/**
  ...
  *
  */
abstract class AbstractAgent[T](configuration: AgentConfig)(
  ...
  protected def initDefaultProducersProperties: Map[String, Properties] = {
    ...
    defaultSerializer match {
      case Some(serializerClass) =>
        propsAgentMessageProducer.put("value.serializer", serializerClass)
      case None =>
        propsAgentMessageProducer.put(
          "value.serializer",
  ...
  protected def initDefaultConsumersProperties: Map[String, Properties] = {
    ...
    defaultDeserializer match {
      case Some(deserializerClass) =>
        propsConsumerAgentMessage.put("value.deserializer", deserializerClass)
      case None =>
        propsConsumerAgentMessage.put(
          "value.deserializer",
  ...
  protected def initProducersProperties: Map[String, Properties] = {
    ...
    initDefaultProducersProperties ++ (configuration.producers match {
      ...
      case Some(producersList) =>
        producersList.foldLeft(Map.empty[String, Properties]) {
          case (mapProp, producerConfig) =>
  ...
  protected def initConsumerProperties: Map[String, Properties] = {
    ...
    initDefaultConsumersProperties ++ (configuration.consumers match {
      ...
      case Some(consumerList) =>
        consumerList.foldLeft(Map.empty[String, Properties]) {
          case (mapProp, consumerConfig) =>
            val props = new Properties()
            props.put("bootstrap.servers", brokers.mkString(","))
  ...
  protected def producers: Map[String, KafkaProducer[String, AgentMessage[T]]] =
    propsForProducers.map {
      case (name, props) =>
  ...
  protected val consumers: Map[String, KafkaConsumer[String, AgentMessage[T]]] =
    propsForConsumers.map {
      case (name, props) =>
  ...
  consumers.foreach {
    case (_, kafkaConsumer) =>
      kafkaConsumer.subscribe(util.Collections.singletonList(TOPIC))
  }

  // TODO: keep?
  override def join(agentId: AgentId) = {
    this.agentMessageConsumer
      .subscribe(util.Collections.singletonList(getTopic(agentId)))
